...
# Crop on top side of the image
class CropTopAndResize(object):
    def __init__(self, desired_size, crop_top_pixels):
        self.desired_size = desired_size
        self.crop_top_pixels = crop_top_pixels

    def __call__(self, img):
        # Crop the top portion of the image
        img = transforms.functional.crop(img, self.crop_top_pixels, 0, img.height - self.crop_top_pixels, img.width)
        
        # Resize the cropped image
        img = transforms.functional.resize(img, self.desired_size)
...
class YoutubeVideoDataset(Dataset):
    def __init__(self, save_dir, frames_per_sample=5, random_horizontal_flip=True, color_jitter=0, with_target=True, image_height=160, image_width=320):
    ...
    def __getitem__(self, index):
        ...
        # Define the desired output size for resizing
        desired_size = (self.image_height, self.image_width)
        crop_top_pixels = 40
        crop_n_resize = CropTopAndResize(desired_size, crop_top_pixels)
