...
def parse_args_and_config():
    ...
    args = parser.parse_args() # this is where the -- stuff command runs from the terminal command (python *.py --stuff stuff)
    args.command = 'python ' + ' '.join(sys.argv)
    # args.log_path = os.path.join(args.exp, 'logs', args.doc)
    args.log_path = os.path.join(args.exp, 'logs')

    # parse config file -- config file is a .yml format file which we load directly
    with open(args.config, 'r') as f:
        config = yaml.load(f, Loader=yaml.FullLoader) # Somehow the directly loaded config is a dictionary typed object
    ...
    new_config = dict2namespace(config) # convert from dictionary object to a Namespace typed object
    ...
    if not args.test and not args.sample and not args.video_gen and not args.fast_fid:
        if not args.resume_training:
            ...
            with open(os.path.join(args.log_path, 'config.yml'), 'w') as f:     # This is where config.yml file is writen to the log_path folder
    ...
    config_uncond = new_config  # Note: cond config and uncond config are exactly the same here, judging from the output of the functional
